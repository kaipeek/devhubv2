module devhub::devcard{
    ...
    fun init(ctx: &mut TxContext) {
      ...
      public entry fun kart_olusturma(
            name: vector<u8>,
            title: vector<u8>,
            img_url: vector<u8>,
            years_of_exp: u8,
      ...
      public entry fun Bilgiler(devhub: &mut DevHub, new_description: vector<u8>, id: u64, ctx: &mut TxContext){
      ...
      public entry fun karti_silmek(devhub: &mut DevHub, id: u64, ctx: &mut TxContext) {
        let user_card = object_table::borrow_mut(&mut devhub.cards, id);
        assert!(tx_context::sender(ctx) == user_card.owner, NOT_THE_OWNER);
        user_card.open_to_work = true;
      } 

      public entry fun update_portnefolio(devhub: &mut DevHub, new_portfolio: vector<u8>, id: u64, ctx: &mut TxContext){
        let user_card = object_table::borrow_mut(&mut devhub.cards, id);
        assert!(tx_context::sender(ctx) == user_card.owner, NOT_THE_OWNER);
